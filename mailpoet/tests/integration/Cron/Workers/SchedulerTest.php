...
class SchedulerTest extends \MailPoetTest {
  ...
  /**
   * Clear the identity map and reload the task + newsletter so they behave like
   * the DB-loaded entities the scheduler processes in production: the task's
   * EXTRA_LAZY queue collection then reads counts / first-subscriber from the
   * database instead of stale in-memory state.
   *
